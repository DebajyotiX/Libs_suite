...
class LibsData:
    ...
    def spectrum_peaks(self, show_plot=None):
        ...
        LibsData.general_peaks_detected_index = peak_index
        ...
        with plt.rc_context({'axes.edgecolor': ((0.8, 0.8, 0.8))}):
            ...
            ax.set_ylabel('Counts', color=(0.5, 0.5, 0.5))
            ax.set_xlabel('Wavelength(nm)', color=(0.5, 0.5, 0.5))
            ax.set_title("Peaks", color=(0.5, 0.5, 0.5))
            ax.tick_params(direction='inout', length=6, width=1.5, colors=(0.5, 0.5, 0.5))
            ax.set_xlim([x[0], x[-1]])
            ax.set_ylim([-4000, 40000])  # set the range here
            ax.grid(color=(0.8, 0.8, 0.8), ls='--', lw=0.25)
    ...
    def selected_peaks(self, show_plot=None):
        ...
        manually_peaks_selected = np.array([246.85, 279.28, 392.46, 396.00, 628.63, 774.75, 280.02])
        # these are selected manually for the test sample- jaggery
        # copied off Arun bhaiya's presentation file :P

        # Checking the nearness of the peaks which were previously detected to the ones manually selected
        # finding index of the wavelength value nearest to each element of manually_peaks_selected array
        index_of_manually_peaks_selected = np.array()

        '''
        ======================= Variable Description ===================================

        1) manually_peaks_selected ---> hand picked and manually typed for the sample,
                            eg CHNO peaks.
        2) index_of_manually_peaks_selected ---> index values of x(wavelength array),
                            at which the manually selected peaks exist.
        3) LibsData.general_peaks_detected_index --->index values of x(wavelength array), 
                            at which these general/algorithmically selected peaks exist.
        
        To get the WAVELENGTH values at the peaks, we have to put these index values in x
         ---> x[index variable]
         
        To get the INTENSITY values at the peaks, we have to put these index values in y
         ---> y[index variable]
        
        '''
        for j in range(len(manual_peak_selection)):
            index = min(range(len(x)), key=lambda i: abs(x[LibsData.general_peaks_detected_index[i]] - manually_peaks_selected[j]))

            np.append(index_of_manually_peaks_selected, index)
